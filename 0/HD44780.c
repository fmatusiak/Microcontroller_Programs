...
//
//-------------------------------------------------------------------------------------------------
void LCD_WriteText_FLASH(const char * text) {
...
void LCD_WriteText_RAM(char * text) {
	while (*text)
		LCD_WriteData(*text++);
}
...
void LCD_GoTo(uint8_t y) {
	/*
	 LCD_WriteCommand(HD44780_DDRAM_SET |  0x20 * y);
	 return;*/
	switch (y) {
	case 0:
		LCD_WriteCommand(HD44780_DDRAM_SET | 0x00);
		break;
	case 1:
		LCD_WriteCommand(HD44780_DDRAM_SET | 0x40);
		break;
	case 2:
		LCD_WriteCommand(HD44780_DDRAM_SET | 0x14);
		break;
	case 3:
		LCD_WriteCommand(HD44780_DDRAM_SET | 0x54);
		break;
	}
