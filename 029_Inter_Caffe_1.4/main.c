...
//========= OUTS ========================
	...
	SETTINGS_AUTOMAT_REVERSE_OUT_LOW;
	HOPPER_PULS_OUT_LOW;
	PAYOUT_OUT_LOW;
...
void configurationInit() {
	...
	FL_pilotes = 0, FLAGA_Send = 0, FL_Caffe = 0, FL_Nabij = 1, FL_Settings_automat =
			1, FL_SA = 1, FL_Hopper_time = 0;

	com_caffe_on = 5, com_off_value = 10, com_start_automat = 3, com_disp = 7, com_disp_caffe =
			9;
	com_caffe_is = 1, com_payout = 2, com_payout_button = 3, com_money_one = 4, com_money_two =
			5, com_money_five = 6, com_money_ten = 7, com_key_switch = 8;
	com_nabijKredyt = 9, com_money_twenty = 10, com_money_fifty = 11, com_money_hundred =
			12;

	//////////Cafe

	FL_keySwitch = 0, FL_PAYOUT = 0, FL_START_PAYOUT = 0, FL_HOPPER = 0, money_pulses =
			0, FL_Hopper_DOWN = 1, FL_Hopper_UP = 0;
	Tmp_time_payout = 0, Tmp_time_hopper_low = 1000, Tmp_time_hopper_high =
			1000, time_hopper = 1000;
	FL_SCREEN = 0;
...
void switchOn() {
	if (receive == com_start_automat) {
		...
		FL_SCREEN = 1;
		_delay_ms(250);
		while (PIN_LED_BUTTON_IN) {
			_delay_ms(1);
			if (!(PIN_LED_BUTTON_IN)) {
				_delay_ms(1);
				if (!(PIN_LED_BUTTON_IN)) {
					_delay_ms(1);
				}
