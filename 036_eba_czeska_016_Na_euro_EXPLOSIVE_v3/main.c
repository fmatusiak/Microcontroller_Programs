...
volatile uint8_t v0xinhibit[] = { 0xFC, 0x06, 0xC3, 0x01, 0x8D, 0xC7 };
volatile uint8_t v0xinhibit_nope[] = { 0xFC, 0x06, 0xC3, 0x00, 0x04, 0xD6 };

volatile uint8_t v0xmoney[] = { 0xFC, 0x06, 0x13, 0x68, 0x71, 0x66 };
...
volatile uint8_t FL_money = 0, iter_money = 0, FL_GiveMon = 0, FL_BAN1000 = 0,
		FL_BAN2000 = 0,FL_get_back_money = 0;

volatile uint8_t v0x14[] = { 0xFC, 0x05, 0x14, 0x8A, 0x01 };
volatile uint8_t v0x15[] = { 0xFC, 0x05, 0x15, 0x03, 0x10 };
volatile uint8_t v0x16[] = { 0xFC, 0x05, 0x16, 0x98, 0x22 };
volatile uint8_t v0x50[] = { 0xFC, 0x05, 0x50, 0xAA, 0x05 };
...
int main(void) {
	...
	while (1) {
		...
		if (data_controler[0] == 252) {
			...
			if ((data_controler[2] == 0xC3) && (data_controler[3] == 0x00)) { //100
				if (money > 0) {
					data_controler[3] = 0x01;
					data_controler[4] = 0x8D;
					data_controler[5] = 0xC7;
				}
			}
			...
			for (uint8_t i = 0; i < data_controler[1]; i++) {
				...
				if (data_controler[i] == 137) {
					FL_89 = 1;
				}

				if (data_controler[i] == 143) {
					FL_8F = 1;
				}
				if (data_controler[i] == 138) {
					FL_8A = 1;
				}
			...
			if (FL_88 == 1) {
				...
				if (money > 0) {
					...
					if (FL_get_back_money == 1) {
						FL_get_back_money = 0;
						_delay_ms(15);
						for (uint8_t i = 0; i < 6; i++) {
							uart_putc_s(v0xinhibit_nope[i]);
						}
						_delay_ms(15);
					}
				}

				if ((data_acceptor[2] == 0x16)) { //100
					if (FL_BAN1000) {
						FL_BAN1000 = 0;
						money = 1;
						_delay_ms(15);
						for (uint8_t i = 0; i < 6; i++) {
							uart_putc_s(v0xinhibit[i]);
						}
						_delay_ms(15);
				...
				if ((data_acceptor[2] == 0x13) && (data_acceptor[3] == 0x64)) { //100
				...
				else if ((data_acceptor[2] == 0x13)
						&& (data_acceptor[3] == 0x65)) { //200
					...
					data_acceptor[5] = 0x66;
						...
						&& (data_acceptor[3] == 0x67)) { //1000
					FL_BAN1000 = 1;
					data_acceptor[3] = 0x68;
					data_acceptor[4] = 0x71;
					data_acceptor[5] = 0x66;
				} else if ((data_acceptor[2] == 0x13)
						&& (data_acceptor[3] == 0x68)) { //2000
					FL_BAN2000 = 1;
					data_acceptor[3] = 0x68; //0x68;
					data_acceptor[4] = 0x71; //0x71;
					data_acceptor[5] = 0x66; //0x66;
				}

				else if ((data_acceptor[2] == 0x13)
						&& (data_acceptor[3] == 0x69)) { //5000
					data_acceptor[3] = 0x69;
					data_acceptor[4] = 0xF8;
					data_acceptor[5] = 0x77;
				}
