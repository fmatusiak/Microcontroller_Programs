...
volatile uint8_t automat_on = 1, tmp_automat = 0, nabij = 0, FL_time = 1, test =
		1, min = 0;
volatile uint16_t time_out, tmp_time = 0, impuls_time = 3;
...
int main(void) {
	...
	while (automat_on) {
		...
		for (i = 2; i < data_acceptor[1]; i++) {
			...
			if ((data_acceptor[i]) == (27) || (data_acceptor[i]) == (251) || (data_acceptor[i]) == (68)
