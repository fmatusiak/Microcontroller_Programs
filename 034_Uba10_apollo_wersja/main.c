...
volatile uint8_t automat_on = 1,tmp_automat = 0,nabij = 0,FL_time = 0;
volatile uint16_t time_out =20000, tmp_time = 0;
...
//===========================
	...
	while(1)
	{
		data_controler[0] = uart_getc();
			...
			/*		for (i = 2; i < data_acceptor[1]; i++)
					{
						if (data_acceptor[i] == 172)
						{
							nabij+=10;
						}
						if (data_acceptor[i] == 216)
						{
							nabij+=5;
						}
						if (data_acceptor[i] == 201)
						{
							nabij+=2;
						}
						if (data_acceptor[i] == 251)
						{
							nabij+=1;
						}
					}*/
...
/*
	...
	if (nabij && 0)
	{
		...
		if ((tmp_time == time_out)&& FL_time == 1)
		{
		FL_time = 0;
		tmp_time = 0;
		PORT_puls_0;
		...
		if ((tmp_time == time_out)&& FL_time == 0)
		{
		tmp_time = 0;
		nabij--;
		}
