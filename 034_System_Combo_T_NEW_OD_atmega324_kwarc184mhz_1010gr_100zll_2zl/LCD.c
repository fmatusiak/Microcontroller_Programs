...
void Write_LCD(uint8_t n) {
	...
	case 2:
		...
		Write_LCD_String_FLASH(PSTR("10zl 20zl 50zl 100zl"), 2);
		Write_LCD_String_FLASH(PSTR(" 10 20 50gr 1 2 5zl "), 3);
	...
	case 3:
		...
		if (Time_Disp <= 5) {
			...
			Write_LCD_String_FLASH(PSTR("  MONET 2zl  : "), 3);
			convert_m(Money_2zl_Atmega);
		...
		if (Time_Disp > 5 && Time_Disp <= 10) {
			...
			if (Hopper_Two_Opto == 0) {
				Write_LCD_String_FLASH(PSTR("                    "), 3);
				Write_LCD_String_FLASH(PSTR("  MONET 2zl  : FULL "), 3);
			}
	...
	case 15:
		...
		Write_LCD_String_FLASH(PSTR("    5zl      2zl    "), 3);
