...
uint16_t EEMEM Money_2zl_EEPROM = 0;
...
void Check_inhbit(void) {
	if ((FL_WPBlock) || (FL_Blocked)) {
		...
		ccTalk_send(COIN_ACCEPTOR, 2, 231, (uint8_t[] ) { 255, 255 }); //252
		_delay_ms(50);
...
void Init_EEProm(void) {
	...
	if (First_Run != 2) {
		...
		eeprom_write_word(&Money_2zl_EEPROM, Money_2zl_Atmega);
	...
	Money_2zl_Atmega = eeprom_read_word(&Money_2zl_EEPROM);
