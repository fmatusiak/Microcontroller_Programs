...
void Set_Sorter(void) {
	...
	if (Hopper_One_Opto == 0) {
		...
		ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 6, 4 });
		_delay_ms_counter(50);
	...
	if (Hopper_Two_Opto == 0) {
		ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 4, 4 });
		_delay_ms_counter(50);
	...
	if (Hopper_Three_Opto == 0) {
		ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 3, 4 });
		_delay_ms_counter(50);
...
void Init_Coin_Acceptor(void) {
	...
	ccTalk_send(COIN_ACCEPTOR, 2, 231, (uint8_t[] ) { 60, 0 }); //252
	_delay_ms(75);

	_delay_ms(75);

	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 1, 1 });//1
	_delay_ms(75);

	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 2, 1 });//1
	_delay_ms(75);

	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 3, 2 });//2
	_delay_ms(75);

	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 4, 3 });//3
	_delay_ms(75);
	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 5, 4 });
	_delay_ms(75);
	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { 6, 5 });//5
	_delay_ms(75);

	for(uint8_t ii = 7;ii <= 254 ;ii++)
	{
	ccTalk_send(COIN_ACCEPTOR, 2, 210, (uint8_t[] ) { ii, 0 });//5
	_delay_ms(75);
	}
...
//			}
			...
			if (((*(datarecive + 6) == 2) || (*(datarecive + 6) == 4) || (*(datarecive + 6) == 1))
					&& (*(datarecive + 5) == 3)) {
				Money_In_Automat += 5;
				Money_50gr_Atmega++;
				Time_out_Money = 0;
			...
			if (((*(datarecive + 6) == 3) || (*(datarecive + 6) == 4)  || (*(datarecive + 6) == 1))
					&& (*(datarecive + 5) == 4)) {
				Money_In_Automat += 10;
				Money_1zl_Atmega++;
				Time_out_Money = 0;
			...
			if (((*(datarecive + 6) == 5) || (*(datarecive + 6) == 4) || (*(datarecive + 6) == 1))
					&& (*(datarecive + 5) == 6)) {
				Money_In_Automat += 50;
				Money_5zl_Atmega++;
				Time_out_Money = 0;
				Money_IN_Liczniki += 10;
			}
		}
...
void Refil(void) {
	...
	if (ccTalk_recive(datarecive, COIN_ACCEPTOR)) {
		if (*(datarecive + 4) != CoinEvent) {
			...
			if ((*(datarecive + 6) == 4) && (*(datarecive + 5) == 4)) {
				Money_1zl_Atmega++;
