...
void Disp_Menu(void) {
	...
	if ((FL_Serwish == 0) && (FL_Adminh == 0)) {
		if (!Money_In_Automat) {
			...
			if (Expense_Atmega <= Money_In_Automat) {
				Time_Disp++;
				if (Time_Disp < 8) {
					Write_LCD(7);
				} else {
...
void Check_inhbit(void) {
	if ((FL_WPBlock) || (FL_Blocked) || (FL_Adminh) || (FL_Serwish)) {
		...
		_delay_ms_counter(50);
		ccTalk_send(BILL_ACCEPTOR, 2, 231, (uint8_t[] ) { 255, 252 });
		_delay_ms_counter(50);
		uart_clear();
	}
...
void _delay_ms_counter(uint16_t ms) {
	...
	if ((Money_IN_Liczniki) || (Money_OUT_Liczniki)) {
		if (ms == 50) {

			if (Money_IN_Liczniki) {
				COUNTER_IN_OUT_HIGH;
			}
			if (Money_OUT_Liczniki) {
				COUNTER_OUT_OUT_HIGH;
			}
