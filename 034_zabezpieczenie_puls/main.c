...
volatile uint8_t value_one = 0, value_two = 0, value_three = 0, value_four = 0,FL_vr =0 ;

volatile uint8_t version [] = {252,46,136,85,40,80,79,76,41,45,49,48,45,83,83,32,73,68,48,48,51,45,48,51,86,50,51,48,45,52,51,32,49,48,83,69,80,49,50,32,53,54,52,56,38,164};
...
/*
				...
				if (value_one == 136) {
					value_two = uart_getc();
					uart_putc_s(value_two);
					if (value_two == 111) {
						value_three = uart_getc();
						uart_putc_s(value_three);
						if (value_three == 95) {
							for(i = 0; i < 46; i++)
							{
								uart_putc(version[i]);
